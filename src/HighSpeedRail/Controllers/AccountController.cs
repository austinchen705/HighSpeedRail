...
namespace HighSpeedRail.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var user = new AccountDAO().FindUser("austin");
        ...
        public ActionResult Register(RegisterViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var result = _accountDaO.CreateUser(model.UserCode, Encryptor.CalculateMD5Hash(model.Password));
                    if (result == 0)
                        AddErrors(new IdentityResult("register fail"));
                    else
                    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ForgotPassword(HighSpeedRail.Models.RegisterViewModel.ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult ResetPassword(HighSpeedRail.Models.RegisterViewModel.ResetPasswordViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            }
            var user = _accountDaO.FindUser(model.UserCode);
            if (user == null)
            {
                // Don't reveal that the user does not exist
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
            var result = _accountDaO.ResetPassword(model.UserCode, Encryptor.CalculateMD5Hash(model.Password));
            if (result > 0)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
